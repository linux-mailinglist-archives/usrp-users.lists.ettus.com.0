...
Received: from mm2.emwd.com (mm2.emwd.com [172.104.30.75])
	by mail.lfdr.de (Postfix) with ESMTPS id 6838223FF05
	for <lists+usrp-users@lfdr.de>; Sun,  9 Aug 2020 17:29:15 +0200 (CEST)
Received: from [::1] (port=55174 helo=mm2.emwd.com)
	by mm2.emwd.com with esmtp (Exim 4.93)
	(envelope-from <usrp-users-bounces@lists.ettus.com>)
	id 1k4nG3-0004Ky-QG; Sun, 09 Aug 2020 11:29:11 -0400
Received: from ssi-cc-fra1-mro-001.atmailcloud.com ([89.46.80.72]:39204)
 by mm2.emwd.com with esmtps (TLS1.2) tls TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384
 (Exim 4.93) (envelope-from <drtaylor@manx.net>) id 1k4nFz-0004GV-LA
 for usrp-users@lists.ettus.com; Sun, 09 Aug 2020 11:29:07 -0400
DKIM-Signature: v=1; a=rsa-sha256; q=dns/txt; c=relaxed/relaxed; d=manx.net;
 ...
 bh=g1q3i4HdNs+XQ2xDSvqBRReFOPCJaFVFRdFAND8aKnE=; b=WbazZg1ZWJMIAAet6NTYZcVrC3
 WlWG5kfUtJYDEX5BreZZu2xERmhrOZUmX5ChOAadEAltXBzKpjuJl7SJLkD2ygxjE3rzwDhdKnq/s
 O5Wic18XsDd5idqHDwvHs4uEWOpoxVqoFzqHhprY7KWdC3qoI6Hjh+WNNyGpvPnr5Dng=;
Received: from pc2-cc-fra1-mrr-004.internal.atmailcloud.com ([10.20.30.48])
 ...
 id 1k4nFJ-0006z2-Bf; Mon, 10 Aug 2020 01:28:25 +1000
Received: from pc2-cc-fra1-mrc-004.internal.atmailcloud.com ([10.20.30.54])
 ...
 (TLSv1.2:ECDHE-RSA-AES256-GCM-SHA384:256) (Exim 4.92.3)
 (envelope-from <drtaylor@manx.net>)
 id 1k4nFI-0006QS-SH; Mon, 10 Aug 2020 01:28:25 +1000
Received: from [178.16.9.227] (helo=PC1)
 by pc2-cc-fra1-mrc-004.internal.atmailcloud.com with esmtpsa
 (TLSv1.2:ECDHE-RSA-AES256-GCM-SHA384:256) (Exim 4.92.3)
 (envelope-from <drtaylor@manx.net>)
 id 1k4nFI-0005No-3l; Mon, 10 Aug 2020 01:28:24 +1000
Message-ID: <CB1869595CFE49A2B3B40817F30D733E@PC1>
...
References: <3106CD4CFDE84EF2B512E9D68DC0AFF8@PC1>
 ...
 <9ee306dd-8ad4-3c12-df3f-effbd3f5410c@comcast.net>
In-Reply-To: <9ee306dd-8ad4-3c12-df3f-effbd3f5410c@comcast.net>
Date: Sun, 9 Aug 2020 16:28:22 +0100
MIME-Version: 1.0
X-Priority: 3
...
Content-Type: multipart/mixed; boundary="===============1921490919002754696=="
...
Content-Type: multipart/alternative;
	boundary="----=_NextPart_000_000D_01D66E6A.19A8CD10"

This is a multi-part message in MIME format.

------=_NextPart_000_000D_01D66E6A.19A8CD10
...
All fine now in the binding and build process.
Incidentally the pybind11 test switch =3DOFF is included in the =
specified v2.4.3 source building script of the 3.9 OOT Module Porting =
Guide at 21st July.
So nearly there.

I have imported and built a couple of simple previously working 3.7.11 =
python and C++ OOT block scripts into my =91gr-development=92 directory.
I have manually converted the xml files to yml and the blocks import and =
initialise parameters correctly in the GRC and the generated python.
There are presently no call-back functions to further complicate =
matters.

ModuleNotFoundError:No module names =91development=92

id: development_gold_lfsr_py_ff
lavel: Gold LFSR
category: =91[development]=92
.

templates:
  imports: import development
  make: development_gold_lfsr_py_ff(${prn})

1). I am not entirely sure if the problem lies with the .yml files, =
although most of the aforementioned script is auto generated without =
modification from gr_modtool add <..> ?
There are some excellent reference .yml examples to be found in =
/usr/share/gnuradio/grc/blocks.
2). PYTHONPATH =96 I have checked the basics with out success.
...
pybind11 to zero by adding -DPYBIND11_TEST=3DOFF to the cmake command =
line.
...
/usr/lib/cmake/pybind11).
  ...
  On 8/2/20 10:20, David Taylor (manx.net) wrote:
    ...
    I have persevered with the GRC 9.0 installation and have followed =
the 3.9 OOT module porting guide.
...
MPIR_INCLUDE_DIR)=20
    ...
    For VOLKGNSSSDR, the techniques you used for 3.7.11 should still =
work fine.
...
be found elsewhere in the history.
      ...
      I have been upgrading the GRC from 3.7.11 (with Ubuntu 18.04) to =
GRC 9.0 (with Ubuntu 20.04 LTS). See previous dialogue below, using =
3.8.1.0~rc12build2
...
functional using an available B210 over USB3.
      ...
      The problem arises with OOT blocks transfer, CMake in its =
=93modern form=94 and with the new dependency of MPIR.

      1). MPIR library has been installed independently and built at =
V3.0 , with libraries and header located in /usr/local/lib and =
...
and a find package reference added to CMakeLists.txt.
      ...
      I have yet to check whether {VOLK_GNSSSDR_LIBRARIES} has to be =
added to the target_link_libraries, noting the significantly different =
...
dependency addition!
      ...
      Is there a more elegant and better way of incorporating =
VOLKGNSSSDR and the MPIR libraries using modern CMake?
...
upgraded the host side of things, complete with host-resident images,
        and getting a "mis-match" error with B2xx.  The UHD code does =
NOT attempt to re-load the FPGA image if the host side is
...
new gr_modtool and transitioning the 3.7x OOT blocks
        The GNU Radio 3.8 OOT Module Porting Guide looks helpful at 16 =
May 2020.
...
advice please to ensure project conclusion.
          ...
          1). GRC version 3.8.1.0~rc12build2 works standalone and =
appears to have similar Cmake files structure and content. (3.9.0.0 is =
listed in the package manager as available, but with significant and =
noticeable changes in the software migration and dependencies)?=20

          2). Libuhd-dev at 3.15.0.0-2build5 correctly identifies the =
B210 over USB3. (I note that library-file libuhd003 no longer forms part =
of this package).
...
got 14.=94
          ...
          This issue was solved under V3.7x  simply by replacement of =
the FPGA image from archive.=20
...
with us.
        ...
        Also, because I didn't see anything in your work-log about it, =
for N210, you have to run:

        uhd_image_loader --args addr=3D<addr-of-n210>,type=3Dn200

        This loads the appropriate image into the EEPROM of the N210.  =
The N2xxx series, unlike the B2xx series don't do this dynamically at
          runtime.  Once you load an image into them, that image is =
there until it is reprogrammed, even across power-off.  This is =
different than
...
<DIV>Ron,</DIV>
<DIV>&nbsp;</DIV>
<DIV>All fine now in the binding and build process.</DIV>
<DIV>Incidentally the pybind11 test switch =3DOFF is included in the =
specified=20
v2.4.3 source building script of the 3.9 OOT Module Porting Guide at =
21st=20
July.</DIV>
<DIV>So nearly there.</DIV>
<DIV>&nbsp;</DIV>
<DIV>I have imported and built a couple of simple previously working =
3.7.11=20
python and C++ OOT block scripts into my =91gr-development=92 =
directory.</DIV>
<DIV><FONT color=3D#000000>I have manually converted the xml files to =
yml and the=20
blocks import and initialise parameters correctly in the GRC and the =
generated=20
python.</FONT></DIV>
<DIV><FONT color=3D#000000>There are presently no call-back functions to =
further=20
complicate matters.</FONT></DIV>
<DIV><FONT color=3D#000000></FONT>&nbsp;</DIV>
<DIV><FONT color=3D#ff0000>ModuleNotFoundError:No module names=20
=91development=92</FONT></DIV>
<DIV>&nbsp;</DIV>
<DIV>id: development_gold_lfsr_py_ff</DIV>
<DIV>lavel: Gold LFSR</DIV>
<DIV>category: =91[development]=92</DIV>
<DIV>.</DIV>
<DIV>&nbsp;</DIV>
<DIV>templates:</DIV>
<DIV>&nbsp; imports: import development</DIV>
<DIV>&nbsp; make: development_gold_lfsr_py_ff(${prn})</DIV>
<DIV>&nbsp;</DIV>
<DIV>1). I am not entirely sure if the problem lies with the .yml files, =

although most of the aforementioned script is auto generated without=20
modification from gr_modtool add &lt;..&gt; ?</DIV>
<DIV>There are some excellent reference .yml examples to be found in=20
/usr/share/gnuradio/grc/blocks.</DIV>
<DIV>2). PYTHONPATH =96 I have checked the basics with out =
success.</DIV>
<DIV>&nbsp;</DIV>
...
<DIV><B>Sent:</B> Monday, August 3, 2020 10:14 PM</DIV>
<DIV><B>To:</B> <A=20
...
"Calibri"; FONT-WEIGHT: normal; COLOR: #000000; FONT-STYLE: normal; =
DISPLAY: inline'>
<P>On Ubuntu 20.04, you could have just installed pybind11 with apt-get. =
But=20
building from source is fine. BTW, you can reduce the build time of =
pybind11 to=20
zero by adding <SPAN class=3D"blob-code-inner blob-code-marker"=20
data-code-marker=3D"+">-DPYBIND11_TEST=3DOFF to the cmake command =
line.</SPAN></P>
<P><SPAN class=3D"blob-code-inner blob-code-marker" =
data-code-marker=3D"+">Looks=20
like you're missing liborc.</SPAN></P>
<P><SPAN class=3D"blob-code-inner blob-code-marker" =
data-code-marker=3D"+">sudo=20
apt-get install liborc-0.4-dev</SPAN></P>
<P><SPAN class=3D"blob-code-inner blob-code-marker"=20
data-code-marker=3D"+">Ron<BR></SPAN></P>
<DIV class=3Dmoz-cite-prefix>On 8/3/20 10:31, David Taylor (manx.net)=20
wrote:<BR></DIV>
<BLOCKQUOTE cite=3Dmid:5362CD13D5E543729D8D7269B3E9961F@PC1 =
type=3D"cite">
  <DIV dir=3Dltr>
...
#000000">
  ...
  completed the pybind11 build process.</DIV>
  <DIV>&nbsp;</DIV>
  <DIV>Incidentally, my install paths are /usr/local/include/pybind11/ =
and=20
...
#000000">
    ...
    <DIV>&nbsp;</DIV>
    <DIV>I have persevered with the GRC 9.0 installation and have =
followed the=20
    3.9 OOT module porting guide.</DIV>
...
first):</FONT></DIV>
    ...
    python/bindings/CMakeLists.txt:23 (GR_PYBIND_MAKE_OOT)</FONT></DIV>
    <DIV>&nbsp;</DIV>
    <DIV>I added my own version of Findpybind11.cmake, which may of =
course be=20
    contributory.</DIV>
    <DIV>&nbsp;</DIV>
...
fundamental with=20
    modern cmake.</DIV>
    <DIV>VOLKGNSSSDR is pulled successfully in using the old =
method.</DIV>
    <DIV>&nbsp;</DIV>
    <DIV>Many thanks,</DIV>
    <DIV>&nbsp;</DIV>
...
Checking for=20
    module 'gmp'<BR>--&nbsp;&nbsp; No package 'gmp' found<BR>-- Found =
GMP:=20
    /usr/lib/x86_64-linux-gnu/libgmpxx.so&nbsp; <BR>-- Checking for =
module 'mpir=20
    &gt;=3D 3.0'<BR>--&nbsp;&nbsp; No package 'mpir' found<BR>-- Could =
NOT find=20
    MPIR (missing: MPIRXX_LIBRARY MPIR_LIBRARY MPIR_INCLUDE_DIR) <BR>-- =
Found=20
    MPLIB: /usr/lib/x86_64-linux-gnu/libgmpxx.so&nbsp; <BR></P>
...
work=20
    fine.</P>
    <P>Here's an example of an external library from one of my =
OOT's.</P>
...
cmake/Modules/FindPcap.cmake</A><BR></P>
    <P>Ron W6RZ<BR></P>
    <DIV class=3Dmoz-cite-prefix>On 7/26/20 07:06, David Taylor =
(manx.net) via=20
...
scope, or if=20
      ...
      style=3D"FONT-SIZE: 11pt">I have been upgrading the GRC from =
3.7.11 (with=20
...
form=94 and=20
      with the new dependency of MPIR.</P>
      <P class=3DMsoNormal style=3D"MARGIN: 0cm 0cm 10pt; LINE-HEIGHT: =
13pt"><FONT=20
...
/usr/local/lib=20
      and /usr/local/include respectively.</FONT></P>
      <P class=3DMsoNormal style=3D"MARGIN: 0cm 0cm 10pt; LINE-HEIGHT: =
13pt"><FONT=20
...
tested=20
      ...
      <P class=3DMsoNormal style=3D"MARGIN: 0cm 0cm 10pt; LINE-HEIGHT: =
13pt"><FONT=20
      style=3D"FONT-SIZE: 11pt">I have yet to check whether=20
      {VOLK_GNSSSDR_LIBRARIES} has to be added to the =
target_link_libraries,=20
      noting the significantly different approach now used in ..=20
      //my_development/lib/CMakeLists.txt</FONT></P>
      <P class=3DMsoNormal style=3D"MARGIN: 0cm 0cm 10pt; LINE-HEIGHT: =
13pt"><FONT=20
      style=3D"FONT-SIZE: 11pt">3). The standard VOLK library is found =
in CMake=20
      configure as it is now part of the v 9.0 build package. =
</FONT></P>
      <P class=3DMsoNormal style=3D"MARGIN: 0cm 0cm 10pt; LINE-HEIGHT: =
13pt"><FONT=20
...
MPIR=20
      problem please?</FONT></P>
      <P class=3DMsoNoSpacing style=3D"MARGIN: 0cm 0cm 0pt">&nbsp;</P>
      <P class=3DMsoNoSpacing style=3D"MARGIN: 0cm 0cm 0pt"><FONT=20
      style=3D"FONT-SIZE: 11pt">As an OOT build requirement at V 9.0., I =
would=20
      have expected expected the gr-modtool to generate some =
boiler-plate code=20
      to cover this dependency addition!</FONT></P>
      <P class=3DMsoNoSpacing style=3D"MARGIN: 0cm 0cm 0pt">&nbsp;</P>
      <P class=3DMsoNoSpacing style=3D"MARGIN: 0cm 0cm 0pt"><FONT=20
      style=3D"FONT-SIZE: 11pt" color=3D#ff0000>Is there a more elegant =
and better=20
      way of incorporating VOLKGNSSSDR and the MPIR libraries using =
modern=20
      CMake?</FONT></P>
      <P class=3DMsoNoSpacing style=3D"MARGIN: 0cm 0cm 0pt">&nbsp;</P>
      <P class=3DMsoNoSpacing style=3D"MARGIN: 0cm 0cm 0pt"><FONT=20
      color=3D#ff0000>Would reversion to 3.8.1 help in the short term,=20
      noting&nbsp; that the output from my work might benefit and the =
later GRC=20
      scheduler improvements?</FONT></P>
      <P class=3DMsoNoSpacing style=3D"MARGIN: 0cm 0cm 0pt">&nbsp;</P>
...
then=20
        ...
        <DIV>&nbsp;</DIV>
        <DIV>I have only managed to try this with a B210 as the other=20
        transceivers remain in the laboratory under Covid19 university =
building=20
        closure measures.</DIV>
...
the=20
      ...
      with B2xx, if they already have a loaded FPGA image, they won't =
try to=20
      re-load from your host during start-up, unless you<BR>&nbsp; =
power-cycle=20
      them first.&nbsp; So, this can result in you having upgraded the =
host side=20
      of things, complete with host-resident images,<BR>&nbsp; and =
getting a=20
      "mis-match" error with B2xx.&nbsp; The UHD code does NOT attempt =
to=20
      re-load the FPGA image if the host side is<BR>&nbsp; newer than =
the code=20
      resident on the B2xx--only after a power-cycle.<BR>
      <BLOCKQUOTE cite=3Dmid:07FCC8E1668F4D4A814BC4DA01DCA8BE@PC1 =
type=3D"cite">
        <DIV dir=3Dltr>
        <DIV style=3D"FONT-SIZE: 12pt; FONT-FAMILY: 'Calibri'; COLOR: =
...
toolchain and=20
        new gr_modtool and transitioning the 3.7x OOT blocks</DIV>
        <DIV>The GNU Radio 3.8 OOT Module Porting Guide looks helpful at =
16 May=20
        2020.</DIV>
        <DIV>The only real issue I had before was to include=20
...
DISPLAY: inline'>
        ...
        <DIV style=3D"BACKGROUND: #f5f5f5">
        <DIV style=3D"font-color: black"><B>From:</B> <A=20
        title=3Dusrp-users@lists.ettus.com =
moz-do-not-send=3D"true">Marcus D. Leech=20
        via USRP-users</A> </DIV>
...
"Calibri"; FONT-WEIGHT: normal; COLOR: #000000; FONT-STYLE: normal; =
DISPLAY: inline'>
        <DIV class=3Dmoz-cite-prefix>On 06/22/2020 02:45 PM, David =
Taylor=20
        (manx.net) via USRP-users wrote:<BR></DIV>
...
#000000">
          ...
          style=3D"MARGIN: 0cm 0cm 10pt; LINE-HEIGHT: 13pt"><FONT=20
...
stable=20
          ...
          conclusion.</FONT></P>
          <P class=3DMsoNormal style=3D"MARGIN: 0cm 0cm 10pt; =
LINE-HEIGHT: 13pt">The=20
          issues:-</P>
          <P class=3DMsoNormal=20
          style=3D"MARGIN: 0cm 0cm 10pt; LINE-HEIGHT: 13pt"><FONT=20
          style=3D"FONT-SIZE: 11pt">1). GRC version <FONT =
color=3D#0000ff=20
          size=3D3>3.8.1.0~rc12build2</FONT> works standalone and =
appears to have=20
          similar Cmake files structure and content. (<FONT =
color=3D#0000ff=20
          size=3D3>3.9.0.0</FONT> is listed in the package manager as =
available,=20
          but with significant and noticeable changes in the software =
migration=20
          and dependencies)? </FONT></P>
          <P class=3DMsoNormal=20
          style=3D"MARGIN: 0cm 0cm 10pt; LINE-HEIGHT: 13pt"><FONT=20
...
over=20
          USB3. (I note that library-file libuhd003 no longer forms part =
of this=20
          package).</FONT></P>
          <P class=3DMsoNormal=20
          style=3D"MARGIN: 0cm 0cm 10pt; LINE-HEIGHT: 13pt"><FONT=20
...
compatibility,=20
          which I have seen before in 3.7x GRC.<SPAN=20
          style=3D"mso-spacerun: yes">&nbsp; =93</SPAN>Expected FPGA =
compatibility=20
          number 16 expected got 14.=94</FONT></P>
          <P class=3DMsoNormal=20
          style=3D"MARGIN: 0cm 0cm 10pt; LINE-HEIGHT: 13pt"><FONT=20
...
with=20
        your N210 or B2xx?&nbsp; It isn't clear.<BR><BR>
        <BLOCKQUOTE cite=3Dmid:3106CD4CFDE84EF2B512E9D68DC0AFF8@PC1 =
type=3D"cite">
          <DIV dir=3Dltr>
          <DIV style=3D"FONT-SIZE: 12pt; FONT-FAMILY: 'Calibri'; COLOR: =
...
version=20
        you think it's running--if you installed from pre-packaged, =
it'll be in=20
...
series,=20
        ...
        runtime.&nbsp; Once you load an image into them, that image is =
there=20
        until it is reprogrammed, even across power-off.&nbsp; This is =
different=20
...
forward to=20
          ...
          <P class=3DMsoNormal=20
          style=3D"MARGIN: 0cm 0cm 10pt; LINE-HEIGHT: 13pt">Best =
regards,</P>
          <P class=3DMsoNormal=20
          style=3D"MARGIN: 0cm 0cm 10pt; LINE-HEIGHT: 13pt"><FONT=20
          ...
          <P class=3DMsoNormal=20
          style=3D"MARGIN: 0cm 0cm 10pt; LINE-HEIGHT: 13pt"><FONT=20
          style=3D"FONT-SIZE: 11pt">Ph.D Researcher, Limerick =
University, Ireland.=20
...
m"=20
    =
moz-do-not-send=3D"true">http://lists.ettus.com/mailman/listinfo/usrp-use=
rs_lists.ettus.com</A><BR></DIV></DIV></DIV></BLOCKQUOTE>
  <HR>
...
<BR></DIV></DIV></DIV></DIV></DIV></DIV></BLOCKQUOTE>
<P>
<HR>
_______________________________________________<BR>USRP-users mailing=20
