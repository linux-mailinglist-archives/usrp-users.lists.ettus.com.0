...
References: <556BBE2D-2BA6-40A3-BFF5-F5FA2351DB06@k5so.com>
 ...
 <0A955663-E38A-4677-BE65-B53476BBD503@k5so.com>
In-Reply-To: <0A955663-E38A-4677-BE65-B53476BBD503@k5so.com>
...
> Yes, they have a 21cm feedhorn with optical cable from the LNA to the 
...
>>> GPSDO board installed.  As an alternate daughter card they also have 
>>> a TVRX2 that could replace the DBSRX2 if they wish.
>>>
>>> This will be fun!
>>>
>>> Joe
...
>>>>>> combinations of r2 or r3 .bit file and firmware and fpga image 
>>>>>> .bin files I use the response when invoking the usrp_image_loader 
>>>>>> is always the same, namely “invalid reply 32 from device”.
>>>>>>
...
>>>>>> files on several fresh installations of Ubuntu 18.04 and 16.04 
>>>>>> the result is always the same in that things proceed normally as 
>>>>>> the various documents concerning un-bricking an N210 explains, 
>>>>>> until the step of using the usrp_image_loader is executed, at 
>>>>>> which point a RuntimeError returns stating that the “invalid 32 
>>>>>> reply” has occurred.
>>>>>>
>>>>>> I was hopeful that careful use of rev3 .bit and .bin files with 
>>>>>> UHD 3.9.7 would do the trick but alas that is not the case.
>>>>>>
>>>>>> I suspect that you are near the bottom of the list of suggestions 
>>>>>> for me and I do appreciate the time and thinking you have 
>>>>>> afforded me on this issue.  If there is anything remaining to try 
>>>>>> I’d be most willing to try it.
>>>>>>
>>>>>> BTW, the suggestion made by someone earlier to try holding the 
>>>>>> safe button down during the loading of the FPGA from iMPACT 
>>>>>> causes the programming to fail (as reported by iMPACT), so that’s 
>>>>>> apparently not a good thing to do.  But one can recover from that 
>>>>>> state by simply by re-programming with the safe button not held 
>>>>>> but the fundamental problem with the uhd_image_loader step in the 
...
>>>>>>> 3) step initially and seeing if your current loaded image or 
>>>>>>> safe image is capable of being upgraded …it likely is since that 
>>>>>>> protocol is widely compatible across UHD variants. The key here 
>>>>>>> I have to emphasize (since you appear to have been previously 
...
>>>>>>> UHD installation is the only one on your system, and that you 
>>>>>>> have the binary images that are version matched with it…people 
>>>>>>> often get caught out by reminants of various UHD versions 
>>>>>>> installed in various system directories from different install 
>>>>>>> methods.
>>>>>>> -Ian
>>>>>>>
...
>>>>>>>> with using ISE iMPACT to load the FPGA with .bit code and even 
>>>>>>>> how to create the .bit from the associated .bin file and did 
>>>>>>>> try doing that earlier but perhaps not identically to your 
>>>>>>>> prescribed steps below.  I’ll revisit it.  I successfully built 
>>>>>>>> UHD 003_009_000 earlier so I can probably also successfully 
>>>>>>>> build UHD 003_009_007 too.  I’ll do that and give it a go.  I 
>>>>>>>> am familiar with the documents you mentioned.  Generally things 
>>>>>>>> have gone exactly as described right up until UHD needs to 
>>>>>>>> communicate with the stored images at which point it never 
>>>>>>>> does; so far anyway.
>>>>>>>>
...
>>>>>>>>> you can ping 192.168.10.2 in safe mode tell’s you that the 
>>>>>>>>> FPGA has loaded an image from Flash, that it’s passed CRC and 
>>>>>>>>> booted the embedded micro controller on the FPGA and run the 
>>>>>>>>> firmware that replies to ICMP packets…that’s a sign the 
>>>>>>>>> hardware is in reasonable shape, regardless of what actually 
>>>>>>>>> version of image ins currently loaded. If you had the internal 
>>>>>>>>> UART connected to a 3.3V interface you would be seeing the 
>>>>>>>>> FPGA and FW compatibility numbers for this image printed at 
>>>>>>>>> boot if it got this far.
...
>>>>>>>>> 3.9.x version is pretty ideal, it’s well supported in 
>>>>>>>>> Gnuradio, is perhaps the most stable, and has N210 support. If 
>>>>>>>>> you are building UHD yourself from GitHub, then checkout the 
>>>>>>>>> tag “release_003_009_007”.
>>>>>>>>> (Note there is no reason to look for old UHD versions to 
>>>>>>>>> support your H/W, the N210 specific code has changed very 
>>>>>>>>> little for some time, but you will benefit from much improved 
>>>>>>>>> general UHD functionality and much better community support)
...
>>>>>>>>> UHD. Use an R3 .bit file (Stay away from R4 images since we 
>>>>>>>>> know that is electrically incompatible with your H/W) and load 
>>>>>>>>> this via JTAG. Verify you can ping this once it’s loaded. 
>>>>>>>>> Remember this is a volatile load, no flash has changed yet, if 
>>>>>>>>> you reset the H/W this download is lost. The goal now is to 
>>>>>>>>> use the embedded firmware in this JTAG image to load the same 
>>>>>>>>> images in .bin format via the ethernet network and update both 
>>>>>>>>> slot’s in the flash memory with non-volatile images that load 
...
>>>>>>>>> should be in sync, power cycling H/W should work, and tools 
>>>>>>>>> like “uhd_usrp_probe” should find the USRP and print it’s 
>>>>>>>>> detailed H/W config. There are a few common useful things to 
>>>>>>>>> check in the “Troubleshooting” section if things still don’t 
>>>>>>>>> seem to have worked.
>>>>>>>>>
>>>>>>>>> -Ian
>>>>>>>>>
...
>>>>>>>>>> newer .bit files into the FPGA than I previously have tried 
>>>>>>>>>> (I tried the current version of UHD and N210 
>>>>>>>>>> usrp_n210_r4_fpga.bit to no avail) to see if any of them are 
>>>>>>>>>> compatible.  I also was able to build UHD 3.9.0 on a 
...
>>>>>>>>>>> load a more modern FPGA image. It's possible you have to 
>>>>>>>>>>> hold down the safe mode button while loading the image.
>>>>>>>>>>>
...
>>>>>>>>>>>     As the hardware change to rev4 occurred around mid 2011
>>>>>>>>>>>     and the earliest UHD version that exists on the
>>>>>>>>>>>     files.ettus.com/uhd <http://files.ettus.com/uhd> page is
>>>>>>>>>>>     Feb 2104, what is the likelihood in your opinion that
>>>>>>>>>>>     the UHD version will be compatible with the rev2/3
>>>>>>>>>>>     hardware from 2011?
>>>>>>>>>>>
>>>>>>>>>>>     So far I’ve not been successful in reviving the 2014 UHD
>>>>>>>>>>>     version so I’m asking to determine whether continued
>>>>>>>>>>>     effort to do so is likely to yield anything positive
>>>>>>>>>>>     with respect to interfacing with the 2011 hardware.
>>>>>>>>>>>
>>>>>>>>>>>     Joe
>>>>>>>>>>>
...
>>>>>>>>>>>>     literally pulled an old hard drive out of a closet, and
>>>>>>>>>>>>     found a copy of the old hardware repository from back
>>>>>>>>>>>>     in the days when N210 was called "USRP2+". Best that I
>>>>>>>>>>>>     can tell, we only ever released two versions to the
>>>>>>>>>>>>     public. We might have sold R3 stickered as R2 -- I
>>>>>>>>>>>>     don't see anything in the repository that would
>>>>>>>>>>>>     indicate otherwise. Rev 2/3 was sold until around June
>>>>>>>>>>>>     or July 2011, when we moved to rev 4. The only
>>>>>>>>>>>>     firmware/host code changes I can see between any of the
>>>>>>>>>>>>     versions are that R4 used LVDS clocking to the
...
>>>>>>>>>>>>     firmware/fpga image arrangement that we later arrived
>>>>>>>>>>>>     at. The hardware is still fine, but you might be in for
>>>>>>>>>>>>     a bit of a deep dive to get it all running again.
>>>>>>>>>>>>
>>>>>>>>>>>>     If you have a TTL-serial adapter or a logic analyzer
>>>>>>>>>>>>     that works as such, you can connect to the debug UART
>>>>>>>>>>>>     header and get printout information from the firmware
>>>>>>>>>>>>     at boot time. Another good idea would be to take a
>>>>>>>>>>>>     video of the front panel LEDs as you apply power -- the
>>>>>>>>>>>>     boot LED lights give good indication of the
>>>>>>>>>>>>     firmware/FPGA image loading process.
>>>>>>>>>>>>
>>>>>>>>>>>>     Nick
>>>>>>>>>>>>
...
>>>>>>>>>>>>         N210 he has it would seem unlikely that all three
>>>>>>>>>>>>         of the N210 are broken.  That being said and
>>>>>>>>>>>>         considering what you jus said it seems that I
>>>>>>>>>>>>         should’ve been able to find some version of UHD
>>>>>>>>>>>>         that will successfully communicate with the
>>>>>>>>>>>>         firmware and fpga images stored in the unit;  I
>>>>>>>>>>>>         have not, using UHD versions from 3.9.0 to 3.14.0.
>>>>>>>>>>>>
>>>>>>>>>>>>         I wanted to try with even earlier versions of UHD
>>>>>>>>>>>>         but am finding trouble in utilizing UHD v3.4.0
>>>>>>>>>>>>         (earliest version I could find) as it seems that
>>>>>>>>>>>>         “prebuilt” v3.4.0 needs only Ubuntu 10.10 or 11.10
>>>>>>>>>>>>         which so far I’m not able to successfully install
>>>>>>>>>>>>         and run.   Seems we’re running out of option on
>>>>>>>>>>>>         this one so the Deep Space Exploration Society, who
>>>>>>>>>>>>         I’m trying to help with this, may have to come to
>>>>>>>>>>>>         grasps with the notion that their N210 is a true
>>>>>>>>>>>>         brick.
>>>>>>>>>>>>
>>>>>>>>>>>>         Joe
>>>>>>>>>>>>
...
>>>>>>>>>>>>>         <mailto:usrp-users@lists.ettus.com>> wrote:
>>>>>>>>>>>>>
>>>>>>>>>>>>>         On 05/09/2019 04:18 PM, Joe Martin via USRP-users
>>>>>>>>>>>>>         wrote:
>>>>>>>>>>>>>>         Nick, Ian,
>>>>>>>>>>>>>>
>>>>>>>>>>>>>>         If this unit happens to be incorrectly labeled as
>>>>>>>>>>>>>>         a rev 2 N210 and it is actually a rev 3 N210, is
>>>>>>>>>>>>>>         there hope in being able to load rev 3 firmware
>>>>>>>>>>>>>>         and fpga images (which I have located previously
>>>>>>>>>>>>>>         and tried actually) with some available UHD
>>>>>>>>>>>>>>         version? If so, would you be able to tell me
>>>>>>>>>>>>>>         which UHD version(s) might be able to communicate
>>>>>>>>>>>>>>         with it?
>>>>>>>>>>>>>>
>>>>>>>>>>>>>>         Joe
>>>>>>>>>>>>>>
...
>>>>>>>>>>>>>         mean that the hardware is broken, and it's
...
>>>>>>>>>>>>>>>         rev 2, based on the factory label.  Is this N210
>>>>>>>>>>>>>>>         a lost cause if it is actually a Rev2 N210?
>>>>>>>>>>>>>>>
>>>>>>>>>>>>>>>         Joe
>>>>>>>>>>>>>>>
...
>>>>>>>>>>>>>>>>>             Ian,
>>>>>>>>>>>>>>>>>
>>>>>>>>>>>>>>>>>             Yes, I have tried many times to boot in
>>>>>>>>>>>>>>>>>             safe mode, same result regardless. Yes, I
>>>>>>>>>>>>>>>>>             am able to pin to 192.168.10.2 successfully.
>>>>>>>>>>>>>>>>>
>>>>>>>>>>>>>>>>>             Joe
>>>>>>>>>>>>>>>>>
>>>>>>>>>>>>>>>>>>             On May 9, 2019, at 1:47 PM, Joe Martin
>>>>>>>>>>>>>>>>>>             via USRP-users
>>>>>>>>>>>>>>>>>>             <usrp-users@lists.ettus.com
>>>>>>>>>>>>>>>>>>             <mailto:usrp-users@lists.ettus.com>> wrote:
>>>>>>>>>>>>>>>>>>
...
>>>>>>>>>>>>>>>>>>             the factory label on the back of the
>>>>>>>>>>>>>>>>>>             N210, showing N210 r:2.0 and 2) a top
...
>>>>>>>>>>>>>>>>>>>             the N210 motherboard. If the SRAM chip
>>>>>>>>>>>>>>>>>>>             is on the top side, it's a rev 2/3. If
>>>>>>>>>>>>>>>>>>>             the SRAM is on the bottom side, it's a
>>>>>>>>>>>>>>>>>>>             rev 4. If you send a picture along of
>>>>>>>>>>>>>>>>>>>             the top of the N210, I can tell you if
>>>>>>>>>>>>>>>>>>>             it's early or late rev.
>>>>>>>>>>>>>>>>>>>
...
>>>>>>>>>>>>>>>>>>>                 little late last night and looked
>>>>>>>>>>>>>>>>>>>                 back through the development
>>>>>>>>>>>>>>>>>>>                 repository for the N210 and as far
>>>>>>>>>>>>>>>>>>>                 as I can tell there was never
>>>>>>>>>>>>>>>>>>>                 customer facing FPGA code for a Rev2
>>>>>>>>>>>>>>>>>>>                 N210. Chatting with Matt
>>>>>>>>>>>>>>>>>>>                 this morning he shared my feeling
>>>>>>>>>>>>>>>>>>>                 that a Rev2 wasn't sold to
>>>>>>>>>>>>>>>>>>>                 customers, so I'm curious if you
>>>>>>>>>>>>>>>>>>>                 have a unit that has a factory label
>>>>>>>>>>>>>>>>>>>                 that says N210Rev2 or if you have
>>>>>>>>>>>>>>>>>>>                 seen "usrp2 rev2.0" on the PCB
...
<html>
  ...
  <body bgcolor="#FFFFFF" text="#000000">
    <div class="moz-cite-prefix">On 05/10/2019 09:29 PM, Joe Martin
      wrote:<br>
    </div>
    ...
    <blockquote cite="mid:0A955663-E38A-4677-BE65-B53476BBD503@k5so.com"
      ...
      <div><br class="">
        <blockquote type="cite" class="">
          <div class="">On May 10, 2019, at 7:17 PM, Marcus D. Leech
          ...
          <div class="">
            ...
            <div bgcolor="#FFFFFF" text="#000000" class="">
              <div class="moz-cite-prefix">On 05/10/2019 09:15 PM, Joe
              ...
              <blockquote
                ...
                <meta http-equiv="Content-Type" content="text/html;
                  charset=utf-8" class="">
                Yes, Nick’s suggestion concerning S2 was certainly the
                key!  I learned a ton about this from all of you guys
                though and do sincerely appreciate the patient and
                helpful manner you all have exhibited to me.  
                <div class=""><br class="">
                </div>
                <div class="">The daughter card is the DBSRX2 that
                  they’ll use for hydrogen-line work and some pulsar
                  work in the 1400MHz range and they also have a GPSDO
                  board installed.  As an alternate daughter card they
                  also have a TVRX2 that could replace the DBSRX2 if
                  they wish.  
                  <div class=""><br class="">
                  </div>
              ...
              I haven't kept up with their progress for years--is the
              "new" site now operational?  It was certainly a "project"
              last time I saw any news of it...<br class="">
              <br class="">
              <br class="">
              <blockquote
                cite="mid:6A14E23C-AAFE-43EF-A207-C830BFD62642@k5so.com"
                ...
                <div class="">
                  <div class="">
                    <div class=""><br class="">
                      <blockquote type="cite" class="">
                        <div class="">On May 10, 2019, at 6:58 PM,
                        ...
                        <div class="">
                          ...
                          <div bgcolor="#FFFFFF" text="#000000" class="">
                            <div class="moz-cite-prefix">On 05/10/2019
                              07:53 PM, Joe Martin via USRP-users wrote:<br
                                class="">
                            </div>
                            <blockquote
                              ...
                              <meta http-equiv="Content-Type"
                                ...
                                class="">
                              Holy smoke!  SUCCESS!!  Nick pointed out
                              that there are two switches on the N210;
                              S1 and S2 and S1 is a reset, so an upload
                              of FPGA code fails if that is held (which
                              I was holding for his suggested test!).
                               Holding S2 during iMPACT loading of the
                              .bit image results in the uhd_image_loader
                              step being SUCCESSFUL!!  I am so happy to
                              see that!  Uhd_usrp_probe works
                              wonderfully.  Finally.  We now can put
                              this little to work doing some tough
                              things!  
                              <div class=""><br class="">
                              </div>
                              <div class="">Thank you all SO MUCH for
                                your assistance with reviving this N210.
                                 A monumental achievement in my book!  
                                I don’t know what to say except that we
                                TOTALLY appreciate you efforts to get us
                                running.  You guys are GREAT! </div>
                              <div class=""><br class="">
                              </div>
                              <div class="">Very best wishes to you each
                                and every one!   </div>
                              <div class=""><br class="">
                              </div>
                            ...
                            I think Nick deserves most of the credit--he
                            was in Ettus R&amp;D during the N210
                            development days.<br class="">
                            <br class="">
                            Hopefully DSES will now have a receiver --
                            what daughtercard do they have in it?<br
                              class="">
                            <br class="">
                            <br class="">
                            <blockquote
                                ...
                                <div class="">
                                  <blockquote type="cite" class="">
                                    <div class="">On May 10, 2019, at
                                      5:30 PM, Joe Martin via USRP-users
                                      &lt;<a moz-do-not-send="true"
                                        href="mailto:usrp-users@lists.ettus.com"
                                        class="">usrp-users@lists.ettus.com</a>&gt;
                                    ...
                                    <div class="">
                                      ...
                                      <div style="word-wrap: break-word;
                                        ...
                                        <div class="">I have been very
                                          careful to avoid the pitfalls
                                          you detailed.  I began with a
                                          fresh installation of Ubuntu
                                          18.04 then performed a
                                          successful build of UHD 3.9.7,
                                          then used command:</div>
                                        <div class=""><br class="">
                                        </div>
                                        ...
                                        <div class="">to load the
                                          appropriate/associated images
                                          into the PC. </div>
                                        <div class=""><br class="">
                                        </div>
                                        <div class="">Then used ISE
                                          iMPACT to load the
                                          “usrp_n210_r3_fpga.bit” file
                                          into the FPGA of the N210.
                                           iMPACT reports “PROGRAM
                                          SUCCESSFUL”. </div>
                                        <div class=""><br class="">
                                        </div>
                                        <div class="">Then without power
                                          cycling the N210 used the
                                          command: </div>
                                        <div class=""><br class="">
                                        </div>
...
—fpga-path=/usr/local/share/uhd/images/usrp_n210_r3_fpga.bin</div>
                                        ...
                                        <div class="">To load the
                                          ...
                                          N210 but I always get the
                                          “RuntimeError: Received
                                          invalid 32 reply from device”
                                          error when running
                                          usrp_image_loader. </div>
                                        <div class=""><br class="">
                                        </div>
                                        <div class="">I am able to
                                          successfully ping 192.168.10.2
                                          but no matter what
                                          combinations of r2 or r3 .bit
                                          file and firmware and fpga
                                          image .bin files I use the
                                          response when invoking the
                                          usrp_image_loader is always
                                          the same, namely “invalid
                                          reply 32 from device”.  </div>
                                        <div class=""><br class="">
                                        </div>
                                        <div class="">The command
                                          uhd_find_devices returns by
                                          reporting it can find a usrp2
                                          device at address
                                          192.168.10.2, as you would
                                          hope.  </div>
                                        <div class=""><br class="">
                                        </div>
                                        <div class="">After trying every
                                          conceivable combination of
                                          these actions with numerous
                                          versions of UHD and r2/r3 .bit
                                          FPGA files and r2/r3 .bin
                                          files on several fresh
                                          installations of Ubuntu 18.04
                                          and 16.04 the result is always
                                          the same in that things
                                          proceed normally as the
                                          various documents concerning
                                          un-bricking an N210 explains,
                                          until the step of using the
                                          usrp_image_loader is executed,
                                          at which point a RuntimeError
                                          returns stating that the
                                          “invalid 32 reply” has
                                          occurred.  </div>
                                        <div class=""><br class="">
                                        </div>
                                        <div class="">I was hopeful that
                                          careful use of rev3 .bit and
                                          .bin files with UHD 3.9.7
                                          would do the trick but alas
                                          that is not the case.  </div>
                                        <div class=""><br class="">
                                        </div>
                                        <div class="">I suspect that you
                                          are near the bottom of the
                                          list of suggestions for me and
                                          I do appreciate the time and
                                          thinking you have afforded me
                                          on this issue.  If there is
                                          anything remaining to try I’d
                                          be most willing to try it. </div>
                                        <div class=""><br class="">
                                        </div>
                                        <div class="">BTW, the
                                          suggestion made by someone
                                          earlier to try holding the
                                          safe button down during the
                                          loading of the FPGA from
                                          iMPACT causes the programming
                                          to fail (as reported by
                                          iMPACT), so that’s apparently
                                          not a good thing to do.  But
                                          one can recover from that
                                          state by simply by
                                          re-programming with the safe
                                          button not held but the
                                          fundamental problem with the
                                          uhd_image_loader step in the
                                          unbricking process always
                                          seems to result. </div>
                                        <div class=""><br class="">
                                        </div>
                                        <div class="">Joe</div>
                                              ...
                                              <div class="">
                                                <div class=""
                                                  ...
                                                  <div class="">To save
                                                    ...
                                                    trying jumping to
                                                    the 3) step
                                                    initially and seeing
                                                    if your current
                                                    loaded image or safe
                                                    image is capable of
                                                    being upgraded …it
                                                    likely is since that
                                                    protocol is widely
                                                    compatible across
                                                    UHD variants. The
                                                    key here I have to
                                          ...
                                          <blockquote type="cite"
                                            ...
                                            <div class="">
                                              ...
                                              <div style="word-wrap:
                                                ...
                                                class="">Joe, 
                                                <div class="">To save
                                                  you time, It may well
                                                  be worth you trying
                                                  jumping to the 3) step
                                                  initially and seeing
                                                  if your current loaded
                                                  image or safe image is
                                                  capable of being
                                                  upgraded …it likely is
                                                  since that protocol is
                                                  widely compatible
                                                  across UHD variants.
                                                  The key here I have to
                                                  emphasize (since you
                                                  appear to have been
                                                  previously getting
                                                  stuck with
                                                  ...
                                                  only one on your
                                                  system, and that you
                                                  have the binary images
                                                  that are version
                                                  matched with it…people
                                                  often get caught out
                                                  by reminants of
                                                  various UHD versions
                                                  installed in various
                                                  system directories
